...
Updated for LIEF >= 0.13.0 compatibility.
...
# Parse LIEF version for compatibility checks
LIEF_MAJOR, LIEF_MINOR, *_ = lief.__version__.split('.')
LIEF_MAJOR, LIEF_MINOR = int(LIEF_MAJOR), int(LIEF_MINOR)
...
class SectionInfo(FeatureType):
    ...
    def raw_features(self, bytez, lief_binary):
        ...
        try:
            # Updated for LIEF >= 0.13.0: Use imagebase property and handle exceptions properly
            if hasattr(lief_binary, 'imagebase'):
                imagebase = lief_binary.imagebase
            else:
...
class ExportsInfo(FeatureType):
    ...
    def raw_features(self, bytez, lief_binary):
        ...
        # Updated for LIEF >= 0.13.0: Handle both old and new export formats
        clipped_exports = []
        try:
            # Try new format first (LIEF >= 0.13.0)
            if hasattr(lief_binary, 'exported_functions'):
                for export in lief_binary.exported_functions:
                    if hasattr(export, 'name') and export.name:
                        clipped_exports.append(export.name[:10000])
...
class GeneralFileInfo(FeatureType):
    ...
    def raw_features(self, bytez, lief_binary):
        ...
        # Updated for LIEF >= 0.13.0: Handle signature checking properly
        has_signature = 0
        ...
        # Handle exported functions counting
        exports_count = 0
        if hasattr(lief_binary, 'exported_functions'):
            exports_count = len(lief_binary.exported_functions)

        # Handle imported functions counting
        imports_count = 0
        if hasattr(lief_binary, 'imported_functions'):
...
class PEFeatureExtractor(object):
    ...
    def raw_features(self, bytez):
        # Updated exception handling for LIEF >= 0.13.0
        lief_errors = (RuntimeError,)
        
        # Add LIEF-specific exceptions if they exist
        try:
            lief_errors = lief_errors + (lief.bad_format,)
        except AttributeError:
            pass
        try:
            lief_errors = lief_errors + (lief.bad_file,)
        except AttributeError:
            pass
        try:
            lief_errors = lief_errors + (lief.pe_error,)
        except AttributeError:
            pass
        try:
            lief_errors = lief_errors + (lief.parser_error,)
        except AttributeError:
            pass
        try:
            lief_errors = lief_errors + (lief.read_out_of_bound,)
        except AttributeError:
            pass
        try:
            lief_errors = lief_errors + (lief.corrupted,)
        except AttributeError:
            pass
        try:
            lief_errors = lief_errors + (lief.conversion_error,)
        except AttributeError:
            pass
            
        try:
            # Updated for LIEF >= 0.13.0: Use bytes directly instead of list conversion
